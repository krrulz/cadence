...
- `timesheetEntries/{id}` — `employeeId, date, yearMonth, clarityId, clarityName, epicId, epicName, hours, weekend, overtime, justification, batchId, createdByUid, createdAt` — one row per Clarity/Epic/day allocation. Normal entries ("Log hours") are auto-split across working days (max 8h/day, skipping weekends and days already full) from a chosen start date; weekend or over-8h/day hours go through a separate "Extra / weekend hours" flow that requires a `justification` (business reason or change number). `batchId` groups the entries created from one split submission.
- `timesheetPeriods/{employeeId}_{yearMonth}` — `employeeId, yearMonth, status ('open'|'frozen'), frozenAt, frozenBy, unlockedAt, unlockedBy` — the freeze state of one employee's calendar month. The employee can freeze their own open month (for month-end invoicing); only admin can unfreeze it. `timesheetEntries` for a frozen month become read-only to the employee (enforced in `firestore.rules` via a `get()` on the matching period doc) — admin can still edit/unlock at any time.
...
- **Timesheets** — employees log hours per Clarity ID + Epic from My Dashboard / the Timesheet tab; hours auto-split across working days (max 8h/day). Weekend or over-8h/day hours require a business justification or change number. Employees can freeze a month for invoicing; only admin can unfreeze it. Admin reviews/exports all direct reports' hours at `/timesheets`, with CSV export (current view or all) for month-end invoicing.
